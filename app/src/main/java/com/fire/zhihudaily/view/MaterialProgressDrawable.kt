...
class MaterialProgressDrawable : Drawable, Animatable {
    ...
    private lateinit var mRing: MaterialProgressDrawable.Ring
    ...
    constructor(context: Context, parent: View) {
        ...
        mRing = MaterialProgressDrawable.Ring(mCallback)
    ...
    fun updateSizes(size: Int) {
        if (size == LARGE) {
            setSizeParameters(CIRCLE_DIAMETER_LARGE.toDouble(), CIRCLE_DIAMETER_LARGE.toDouble(),
                    CENTER_RADIUS_LARGE.toDouble(),
                    STROKE_WIDTH_LARGE.toDouble(), ARROW_WIDTH_LARGE.toFloat(),
                    ARROW_HEIGHT_LARGE.toFloat())
        } else {
            setSizeParameters(CIRCLE_DIAMETER.toDouble(), CIRCLE_DIAMETER.toDouble(),
    ...
    private fun getMinProgressArc(ring: MaterialProgressDrawable.Ring): Float {
     ...
     * the next color.
     */
    private fun updateRingColor(interpolatedTime: Float, ring: MaterialProgressDrawable.Ring) {
    ...
    private fun applyFinishTranslation(interpolatedTime: Float,
            ring: MaterialProgressDrawable.Ring) {
        // shrink back down and complete a full rotation before
        // starting other circles
        // Rotation goes between [0..1].
        updateRingColor(interpolatedTime, ring)
        val targetRotation = (Math.floor(
                (ring.startingRotation / MAX_PROGRESS_ARC).toDouble()) + 1f).toFloat()
            ...
            public override fun applyTransformation(interpolatedTime: Float, t: Transformation) {
                if (mFinishing) {
                    ...
                    if (interpolatedTime > END_TRIM_START_DELAY_OFFSET) {
                        ...
                        val endTrim = startingEndTrim + minArc * MATERIAL_INTERPOLATOR.getInterpolation(
                                scaledTime)
